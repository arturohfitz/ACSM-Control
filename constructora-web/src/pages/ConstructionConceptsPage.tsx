...
export default function ConstructionConceptsPage() {
  ...
  async function loadCatalog(projectId: number | null, modelId: number | null, search = query) {
    ...
    try {
      ...
      setItems(
        await apiRequest<ConceptModelCatalogItem[]>(
          `/construction-concepts/model-catalog?${params}`,
        ),
      )
    } catch (err) {
      setItems([])
      setError(err instanceof Error ? err.message : 'No fue posible cargar conceptos del modelo')
    } finally {
  ...
  async function submitConcept(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'No fue posible guardar el concepto')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="space-y-4">
          <section className="overflow-hidden rounded-[24px] border border-sky-200/80 bg-[linear-gradient(180deg,#f8fcff_0%,#e3f0f8_100%)] shadow-panel">
            ...
            <div className="grid gap-3 border-b border-sky-200/80 p-4 lg:grid-cols-[minmax(0,1fr)_320px]">
              ...
              >
                <label className="mb-2 block text-sm font-bold text-acsm-ink" htmlFor="concept-search">
                  Buscar
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Codigo, concepto o capitulo"
                  />
                </div>
              </form>
            </div>
              ...
              <table className="w-full min-w-[1080px] border-collapse text-sm">
                <thead>
                  <tr className="border-b border-sky-200 bg-sky-100/70 text-left text-xs uppercase text-acsm-muted">
                    ...
                    <th className="px-4 py-3">Importe ref.</th>
                ...
                <tbody>
                  ...
                  {!catalogLoading && items.length === 0 ? (
                    <tr>
                      <td className="px-4 py-10 text-center text-sm font-semibold text-acsm-muted" colSpan={10}>
                        Sin conceptos para el desarrollo y modelo seleccionados.
                      </td>
                    </tr>
                  ) : null}
                  {!catalogLoading
                    ? items.map((item) => (
                        <tr key={item.id} className="border-b border-sky-100 bg-white/70">
                          <td className="px-4 py-3 font-semibold text-acsm-muted">
                            {item.concept_code ?? item.source_code ?? '-'}
                          </td>
                          <td className="px-4 py-3">
                            <div className="font-bold text-acsm-ink">{item.concept_name}</div>
                          ...
                          <td className="px-4 py-3 text-acsm-muted">
                            {item.chapter_name ?? item.chapter_code ?? '-'}
                          </td>
                          ...
                          <td className="px-4 py-3 font-semibold text-acsm-ink">
                            {formatCurrency(item.unit_price_reference)}
                          </td>
                          <td className="px-4 py-3 font-semibold text-acsm-ink">
                            {formatCurrency(item.total_price_reference)}
                          </td>
                          <td className="px-4 py-3">
                            <span
                              className={[
      ...
      >
        <form id="concept-form" onSubmit={submitConcept} className="space-y-4">
          ...
          <label className="block">
            <span className="mb-2 block text-sm font-bold text-acsm-ink">Descripcion</span>
            <textarea
              value={form.description}
              onChange={(event) =>
                setForm((current) => ({ ...current, description: event.target.value }))
              }
              className="min-h-24 w-full rounded-xl border border-sky-200 bg-white px-3 py-2 text-sm font-semibold outline-none focus:border-sky-400 focus:ring-4 focus:ring-sky-100"
            />
          </label>
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                onChange={(event) =>
                  setForm((current) => ({ ...current, default_waste_percent: event.target.value }))
                }
                className="h-11 w-full rounded-xl border border-sky-200 bg-white px-3 text-sm font-semibold outline-none focus:border-sky-400 focus:ring-4 focus:ring-sky-100"
              />
            </label>
            <label className="block">
              ...
              <input
                ...
                onChange={(event) =>
                  setForm((current) => ({ ...current, default_indirect_percent: event.target.value }))
                }
                className="h-11 w-full rounded-xl border border-sky-200 bg-white px-3 text-sm font-semibold outline-none focus:border-sky-400 focus:ring-4 focus:ring-sky-100"
              />
            </label>
